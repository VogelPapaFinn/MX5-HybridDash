...
twai_frame_t* activeMessages_[CAN_QUEUE_DEPTH];
...
static bool transmitOfMessageDoneCb(const twai_node_handle_t nodeHandle,
                                    const twai_tx_done_event_data_t* eventData,
                                    void* userCtx)
{
	// Is it our handle?
	if (nodeHandle != *nodeHandle_) return false;
	...
	for (uint8_t i = 0; i < CAN_QUEUE_DEPTH; i++) {
		...
		if (activeMessages_[i] != NULL) {
			...
			if (activeMessages_[i]->header.id == receivedFrame->header.id &&
				activeMessages_[i]->buffer_len == receivedFrame->buffer_len &&
				activeMessages_[i]->buffer == receivedFrame->buffer) {
...
static bool receivedMessageCb(twai_node_handle_t nodeHandle, const twai_rx_done_event_data_t* eventData, void* user_ctx)
{
	// Is it our handle?
	if (nodeHandle != *nodeHandle_) return false;
	...
	if (amountOfTasksToNotifyOnMsgReceived_ == 0) return false;
...
bool enableCanNode()
{
	if (nodeHandle_ == NULL) return false;
...
bool registerMessageReceivedCb(TaskHandle_t* taskToNotify)
{
	...
	const void* newAddr = realloc(tasksToNotifyOnMsgReceived_,
	                              sizeof(TaskHandle_t*) * (amountOfTasksToNotifyOnMsgReceived_ + 1));
